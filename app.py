...
def check_password():
    ...
    if not st.session_state["password_correct"]:
        st.markdown("""
            <style>
                .stTextInput > div > div > input { background-color: #f0f2f6; color: #000000; }
                .login-form { max-width: 400px; margin: 0 auto; padding: 2rem; border-radius: 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); background-color: white; }
                .login-title { margin-bottom: 2rem; text-align: center; color: #4CAF50; }
...
def logout():
    """Faz logout removendo o cookie de autenticação e reinicia a aplicação"""
    if "auth_cookie" in st.session_state:
        del st.session_state["auth_cookie"]
    st.session_state["password_correct"] = False
    st.session_state["login_attempt"] = False
    st.session_state.messages = []  # Limpa as mensagens ao fazer logout
...
def handle_message():
    ...
    if st.session_state.user_input.strip():
        ...
        if not is_duplicate:
            ...
            with st.chat_message("assistant", avatar=logo_path):
                ...
                with st.spinner():
                    ...
                    if file_content:
                        ...
                        combined_context = f"{rag_context}\n{file_context}" if rag_context else file_context
                    else:
                ...
                if result:
                    ...
                    timestamp = datetime.now().strftime("%H:%M")
                    st.session_state.messages.append({"role": "assistant", "content": assistant_message, "time": timestamp})
                    if is_first_message:
            ...
            st.session_state.user_input = ""  # Limpa o campo de texto após enviar
...
def regenerate_message(index):
    ...
    if result:
        ...
        if index+1 < len(st.session_state.messages) and st.session_state.messages[index+1]["role"] == "assistant":
            st.session_state.messages[index+1] = {"role": "assistant", "content": new_response, "time": datetime.now().strftime("%H:%M")}
        else:
            st.session_state.messages.insert(index+1, {"role": "assistant", "content": new_response, "time": datetime.now().strftime("%H:%M")})
    else:
...
def create_new_chat():
    ...
    st.session_state.chat_history.append({"id": "", "title": st.session_state.chat_title, "messages": []})
...
def handle_option_selection(option):
    """Processa a seleção de uma opção e envia como nova mensagem"""
    st.session_state.user_input = option
    handle_message()

st.markdown("""
    <style>
    .main .block-container { padding-top: 1rem; padding-bottom: 0; max-width: 1200px; }
    .header { position: fixed; top: 0; left: 0; right: 0; background-color: white; z-index: 999; padding: 1rem; border-bottom: 1px solid #e6e6e6; }
    .chat-message { padding: 1rem; border-radius: 0.5rem; margin-bottom: 0.5rem; display: flex; flex-direction: column; }
    .user-message { background-color: #f0f2f6; border-radius: 0.5rem; align-self: flex-end; }
    .assistant-message { background-color: #ffffff; border: 1px solid #e6e6e6; border-radius: 0.5rem; align-self: flex-start; }
    .message-time { font-size: 0.8rem; color: #666; margin-top: 0.5rem; align-self: flex-end; }
    .input-container { position: fixed; bottom: 0; left: 50%; transform: translateX(-50%); width: 90%; max-width: 800px; background-color: white; padding: 1rem; border-top: 1px solid #e6e6e6; z-index: 998; }
    .sidebar .sidebar-content { background-color: #f8f9fa; }
    .primary-button { background-color: #4CAF50 !important; color: white !important; }
    .secondary-button { border: 1px solid #4CAF50 !important; color: #4CAF50 !important; }
    .stButton button { border-radius: 4px; padding: 0.5rem 1rem; font-weight: 500; }
    .message-actions { display: flex; gap: 5px; margin-top: 5px; justify-content: flex-end; }
    .action-button { background-color: transparent; border: none; color: #4CAF50; cursor: pointer; font-size: 12px; padding: 2px 5px; border-radius: 3px; }
    .action-button:hover { background-color: rgba(76, 175, 80, 0.1); }
    .chat-item { display: flex; align-items: center; padding: 0.5rem; border-radius: 0.25rem; margin-bottom: 0.25rem; cursor: pointer; }
    .chat-item:hover { background-color: rgba(76, 175, 80, 0.1); }
    .chat-item.active { background-color: rgba(76, 175, 80, 0.2); font-weight: bold; }
    .chat-item-title { flex-grow: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .chat-title { text-align: center; padding: 1rem; font-size: 1.5rem; font-weight: bold; color: #4CAF50; }
    ::-webkit-scrollbar { width: 8px; height: 8px; }
    ::-webkit-scrollbar-track { background: #f1f1f1; }
    ::-webkit-scrollbar-thumb { background: #4CAF50; border-radius: 4px; }
    ::-webkit-scrollbar-thumb:hover { background: #45a049; }
    .attach-icon { display: flex; align-items: center; justify-content: center; height: 70px; color: #4CAF50; cursor: pointer; font-size: 20px; }
    @import url('https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css');
    .file-attached { background-color: rgba(76, 175, 80, 0.1); border-radius: 4px; padding: 4px; margin-bottom: 5px; font-size: 12px; display: flex; align-items: center; justify-content: space-between; }
    .file-attached i { margin-right: 5px; }
    .remove-file { color: #f44336; cursor: pointer; }
    #MainMenu {visibility: hidden;}
    footer {visibility: hidden;}
    .stDeployButton {display:none;}
    .edit-message-container { display: flex; flex-direction: column; gap: 5px; margin-bottom: 10px; }
    .edit-actions { display: flex; justify-content: flex-end; gap: 5px; }
    .messages-container { height: calc(100vh - 300px); overflow-y: auto; padding-bottom: 100px; }
    .option-button { background-color: #4CAF50; color: white; margin: 5px; padding: 0.5rem 1rem; border-radius: 4px; border: none; cursor: pointer; }
    .option-button:hover { background-color: #45a049; }
    </style>
""", unsafe_allow_html=True)
...
if check_password():
    ...
    with st.sidebar:
        ...
        for idx, chat in enumerate(st.session_state.chat_history):
            ...
            with col1:
                if st.button(f"📝 {chat['title']}", key=f"chat_{idx}", use_container_width=True, help="Clique para abrir esta conversa"):
                    load_chat(idx)
            with col2:
        ...
        if use_rag:
            rag_source = st.radio("Fonte do Contexto", ("Arquivo", "Texto Direto"), index=("Arquivo", "Texto Direto").index(st.session_state.rag_source))
            st.session_state.rag_source = rag_source
            if rag_source == "Arquivo":
                file_type = st.selectbox("Tipo de Arquivo", ("PDF", "TXT", "CSV"), index=("PDF", "TXT", "CSV").index(st.session_state.file_type))
                ...
                direct_text = st.text_area("Inserir Texto de Contexto", value=st.session_state.get('direct_text', ''), height=150, key="direct_text")
            st.divider()
        ...
        st.selectbox("Selecionar Modelo/Contexto", options=model_options, index=model_options.index(st.session_state.selected_model), key="selected_model")
        st.file_uploader("Anexar arquivo", type=["pdf", "txt", "csv", "doc", "docx", "xls", "xlsx"], key="file_to_send", label_visibility="collapsed")
    ...
    with main_col2:
        ...
        with messages_container:
            print(f"DEBUG: Conteúdo de st.session_state.messages: {st.session_state.messages}")
            if not st.session_state.messages:
                st.markdown('<div style="text-align: center; color: #666;">Nenhuma mensagem ainda. Envie uma mensagem para começar!</div>', unsafe_allow_html=True)
            for idx, message in enumerate(st.session_state.messages):
                if st.session_state.editing_message == idx:
                    ...
                    with st.chat_message("user"):
                        ...
                        with col2:
                            if idx+1 < len(st.session_state.messages) and st.session_state.messages[idx]["role"] == "user":
                    ...
                    with st.chat_message("assistant", avatar=logo_path):
                        ...
                        if options:
                            for option in options:
                                if st.button(option, key=f"option_{idx}_{option}", on_click=handle_option_selection, args=(option,)):
                                    pass
        st.markdown("<div style='height: 100px;'></div>", unsafe_allow_html=True)
        st.markdown('<div class="input-container">', unsafe_allow_html=True)
        col1, col2, col3 = st.columns([6, 1, 1])
        with col1:
            st.text_area("Mensagem", placeholder="Digite sua mensagem aqui...", key="user_input", height=70, label_visibility="collapsed")
        with col2:
            st.empty()
        with col3:
